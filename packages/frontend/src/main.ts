...
const configuredWsBaseUrl = import.meta.env.VITE_BACKEND_WS_BASE_URL ?? '';
...
const baseUrl = resolveWebSocketBaseUrl({
  configuredBaseUrl: configuredWsBaseUrl,
...
if (!baseUrl) {
  root.textContent = 'WebSocket接続先が未設定です。VITE_BACKEND_WS_BASE_URL を設定するか、?wsBaseUrl=wss://... を指定してください。';
  throw new Error('WebSocket base URL is not configured. Set VITE_BACKEND_WS_BASE_URL or pass ?wsBaseUrl=wss://...');
}
