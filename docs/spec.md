...
- **MVP の意味**: Minimum Viable Product（必要最小限の機能で成立する最初のリリース）
- **構成**: Cloudflare Durable Objects（1ルーム = 1 DO）を中継にした準 P2P 構成
- **不正対策（初期段階）**: WebSocket 接続時の認証ヘッダー検証 + サーバー側の手番/ルール検証
- **対象プラットフォーム**: まずは PWA のみ

> 補足: ファイル名は実装仕様全体を管理する意図で `spec.md` のままとし、MVP 以降の拡張も同一ファイルで追記する。
