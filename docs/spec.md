...
- **不正対策**: WebSocket 接続時の認証ヘッダー検証 + サーバー側の手番/ルール検証
...
- `docs/spec.md`: 実装全体の方針・責務分離・進め方（この文書）
- `docs/api.md`: WebSocket 通信のメッセージ仕様（型・項目・同期手順）

重複を避けるため、通信メッセージの詳細は `docs/api.md` を正とする。
